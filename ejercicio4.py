#************zona funcion*****************

def pedir_radio():
    radio = float(input("coloque el radio del cilindro"))
    return radio
def pedir_altura():
    altura = float(input("coloque la altura del cilindro"))
    return altura
def definir_pi():
    black = 3.14
    return black
def calcular_volumen(radio,altura, black):
    volumen = (radio**2) *black * altura
    return volumen
def imprimir_datos(radio, altura):
    mensaje = "el radio es:"+ radio
    mensaje = "la altura es:"+ altura
def imprimir_resultado(resultado_volumen):
    print("el volumen del cilindro es:" + str(resultado_volumen))
    
    
    
#********************zona codigo*****************

radio = pedir_radio()
altura=pedir_altura()
black = definir_pi()
volumen = calcular_volumen(radio,altura,black)
resultado = imprimir_resultado(volumen)
